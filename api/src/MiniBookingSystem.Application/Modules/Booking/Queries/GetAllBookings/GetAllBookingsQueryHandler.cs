...
public class GetAllBookingsQueryHandler
    : BaseGetQueryHandler<GetAllBookingsQuery, Booking, BookingDto>
{
    public GetAllBookingsQueryHandler(IUnitOfWork unitOfWork)
        : base(unitOfWork) { }

    protected override IQueryable<Booking> ApplySearch(
        IQueryable<Booking> query,
        string searchTerm
    ) => query.Where(b => b.BookingCode.ToLower().Contains(searchTerm.ToLower()));
